...
###指令使用的分析
MIPS指令的后缀有`i`和`u`，两者可以同时出现。

`i`意味着该指令存在立即数，指令会将立即数作为有符号数，由16位扩展到32位后加入运算。

`u`意味着该指令对操作数进行的无符号运算。

所以，有立即数的必须上带i版本的指令，用不用带u的取决于希望两个操作数之间进行的是怎样的操作。

总之，一旦使用了无符号版本，逆操作就不能使用负数立即数了，如栈的增减。

##二、语义错误检查
用于在语法树构造完毕后对语义错误进行分析报错。

###模块结构分析
代码生成模块，由于目标代码只有一种，对于代码生成过程中需要保存的一系列数据结构，我们都保存在了各个语法树的结点以及MccRobot中，为此增加了不少的成员变量。从职责上来划分的话，语法树的结点其实只应该有那些和它们数据相关的成员，如MccIntLiteral只应该有它所代表的整数常量值，合理的实现应该是单独有一个代码生成模块（一个类），所有的中间信息都该存在那个类的对象中。

语义错误检查模块由于需要涉及到多种错误检查，每一种错误检查都会遍历一遍语法树（出于简单），所以我们单独划出一系列类，对于每类检查，创建一个对象，所有在该类检查过程中需要保存的值，都放在该对象中。

类结构大致设想如下（按照目前分析出来的语义错误列表）：

- MccSemanticErrorChecker：基类。
...
关于语义错误的收集，不能因为一个不致命的错误的出现就导致整个检查过程结束，所以，对于每个类型的检查，都有相应的错误，我们采用收集错误，最后统一进行报告这种的机制，因此，对于错误我们也有与Checker相对应的类结构，大概如下：

- MccSemanticError：基类。
    - MccVariableTypeError：对应于变量类型为VOID。
    - MccMethodDefinitionNoNameError：对应于方法定义的时候参数无名字。
    - (To Update...)
