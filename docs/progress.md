###Todos
- 汇编器的开头需要插入一些固定代码到ROM中，比如说跳转到main的代码。有可能面临要增强汇编器支持的语法。
...
- **2015/01/03**：优化编译器对于if语句无else语句时的跳转语句的生成，取消掉无else时的多余false_label以及无else时if块结尾的跳转语句。
- **2015/01/04**：在汇编器中给BIOS的代码插入增加了点铺垫。
- **2015/01/04**：汇编器增加对十进制数的立即数的处理。